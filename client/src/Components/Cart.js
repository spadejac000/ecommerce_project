...
const Cart = () => {
  ...
  const clearCart = (id) => {
    axios.delete(`http://localhost:5000/api/cart/cartcontent/hello/${id}`).then(response => {
  ...
  const handleToken = async (token) => {
    const response = await axios.post('http://localohost:5000/api/cart/checkout', {
      token
    });
  ...
  const deleteProduct = (productId) => {
    axios.delete(`http://localhost:5000/api/cart/${productId}`).then(response => {
